...
def main():
            ...
            if "review" in st.session_state and review.benchmarks is None:
                ...
                if review.is_youtube:
                    ...
                    images = st.file_uploader(
                        "foobar", accept_multiple_files=True, label_visibility="hidden"
                    )
    ...
    with tab2:
        n = len(input_urls)
        if st.button("Generate Overall Summary") and "reviews" not in st.session_state:
            with st.spinner(
        ...
        if "reviews" in st.session_state:
            ...
            filename_csv = (
                f'reviews_summary_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.csv'
            )
            st.download_button(
                label="Download as CSV",
                data=data,
